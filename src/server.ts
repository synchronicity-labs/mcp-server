...
const SERVER_INSTRUCTIONS =
  'For lipsync requests, prefer create-lipsync. If the user asks an image or video to say text, call voices_get-voices, then create-lipsync with script + voiceId and the image/video URL or Sync assetId. Do not call tts_create for that flow. In ChatGPT, if the user attached media in chat, use upload-media or direct create-lipsync file params and then poll generate_get-generation. Never use open-upload-widget for local video requests; ask the user to attach the video to the chat composer first because the widget local-file bridge may reject video formats before Sync can read them. Use open-upload-widget only for supported local image/audio staging or when the user explicitly asks for the widget. If the user provides a public media URL, pass it directly as imageUrl, videoUrl, or audioUrl. Poll generate_get-generation until COMPLETED and return outputUrl.';
