...
const SERVER_INSTRUCTIONS =
  'For lipsync requests, prefer create-lipsync. If the user asks an image or video to say text, call voices_get-voices, then create-lipsync with script + voiceId and the image/video URL or Sync assetId. Do not call tts_create for that flow. In ChatGPT, if the user attached media in chat, use upload-media or direct create-lipsync file params and then poll generate_get-generation. If the user needs to pick a local video, ask them to attach it to the chat composer first; the widget local-file bridge may reject video formats before Sync can read them. Use open-upload-widget when the user wants the in-app picker or for supported local image/audio staging. If the user provides a public media URL, pass it directly as imageUrl, videoUrl, or audioUrl. Poll generate_get-generation until COMPLETED and return outputUrl.';
