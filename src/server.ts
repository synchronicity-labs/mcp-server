...
export const SERVER_INSTRUCTIONS =
  'create-lipsync accepts exactly one visual input (image or video) and one driver (audio or script). A script requires a voiceId. Public media URLs and existing Sync asset IDs are supported. The tool defaults to sync-3 and groups work in an integration-specific project unless projectName is supplied. upload-media accepts ChatGPT file objects. open-upload-widget accepts local images and audio in ChatGPT, but not video. generate_get-generation reports progress and returns outputUrl when the generation is complete.';
