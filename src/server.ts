...
export const SERVER_INSTRUCTIONS =
  'For lipsync requests, prefer create-lipsync. If the user asks an image or video to say text, call voices_get-voices, then create-lipsync with script + voiceId and the image/video URL or Sync assetId. Do not call tts_create for that flow. In ChatGPT, if the user attached media in chat, use upload-media or direct create-lipsync file params and then call generate_get-generation once with wait: true and timeout: 55. Critical video rule: open-upload-widget is image/audio only. Never call, recommend, or describe open-upload-widget for local video or MP4 requests, even if the user says widget. Never mention requestedMediaType: "video"; it is invalid. For any local MP4/video, the first step is attaching the video to the ChatGPT composer. After it is attached, use upload-media or direct create-lipsync file params. Use open-upload-widget only for supported local image/audio staging or when the user explicitly asks for the widget for image/audio. If the user provides a public media URL, pass it directly as imageUrl, videoUrl, or audioUrl. After generate_get-generation returns COMPLETED, copy the exact structuredContent.outputUrl string verbatim; never reconstruct, shorten, or edit signed result URLs.';
